...
import { useTheme } from "../../context/ThemeContext"; // Context yolunu projenize göre kontrol edin
...
export default function StepExpertiz({ formData, updateField }) {
  ...
  const SelectCell = ({ partKey, value }) => {
    ...
    return (
      <button
        ...
        className={`w-8 aspect-square rounded-md border inline-flex items-center justify-center transition-all active:scale-[0.98] ${
          !active ? (isDark ? "border-slate-700 hover:border-slate-500" : "border-slate-200 hover:border-slate-300") : ""
        }`}
        style={{
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div>
        <h2 className={`text-xl font-semibold mb-1 ${isDark ? "text-white" : "text-slate-900"}`}>
          Ekspertiz Durumu
        </h2>
      ...
      {/* Legend */}
      <div className={`flex items-center justify-center gap-4 py-2 rounded-lg ${isDark ? "bg-slate-800" : "bg-slate-100"}`}>
      ...
      <div className="grid lg:grid-cols-2 gap-4 items-stretch">
        {/* Left: Kroki */}
        <div className={`rounded-lg p-4 border ${isDark ? "bg-slate-900 border-slate-700" : "bg-white border-slate-200"}`}>
          <h3 className={`text-sm font-semibold mb-3 ${isDark ? "text-white" : "text-slate-800"}`}>Araç Krokisi</h3>

          <div className="relative">
            <div className="w-full max-w-[520px] mx-auto h-[380px] md:h-[420px] flex items-center justify-center">
              <svg viewBox="0 0 227 303" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <g transform="translate(1003.000000, 233.000000)">
                      ...
                      <g transform="translate(162.000000, 52.000000)" stroke="#D3D2D2">
                        ...
                        <path
                          ...
                          onClick={() => updateExpertiz("SagArkaCamurluk", cycleValue(getPartValue("SagArkaCamurluk")))}
                      ...
                      >
                        ...
                        <path
                          ...
                          className="cursor-pointer transition-all hover:opacity-80"
                          transform="translate(34.500000, 182.500000) rotate(-90.000000) translate(-34.500000, -182.500000)"
                          onClick={() => updateExpertiz("SolArkaCamurluk", cycleValue(getPartValue("SolArkaCamurluk")))}
            ...
            {/* Sabit kutu */}
            <div
              className={`mt-3 p-2 rounded border text-center transition-all ${
                isDark ? "bg-slate-800" : "bg-slate-50"
              }`}
              style={{
                borderColor: displayKey ? `${displayStatus.color}55` : (isDark ? "rgba(148,163,184,0.25)" : "#e2e8f0"),
              }}
            >
              {!displayKey ? (
                <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-500"}`}>Bir parça seçin</span>
              ) : (
                <>
                  <span className={`text-sm font-semibold ${isDark ? "text-white" : "text-slate-900"}`}>{displayLabel}</span>
                  <span className={`text-xs mx-2 ${isDark ? "text-slate-400" : "text-slate-300"}`}>•</span>
          ...
          <div className="grid grid-cols-3 gap-2 mt-3">
            {expertizOptions.map((option) => {
              ...
              return (
                <div key={option.value} className={`text-center p-2 rounded ${isDark ? "bg-slate-800" : "bg-slate-50"}`}>
                  <div className="text-2xl font-bold" style={{ color: option.color }}>
                    {count}
                  </div>
                  <div className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>{option.label}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className={`rounded-lg border overflow-hidden flex flex-col ${
          ...
          <div className={`px-3 py-2 border-b ${isDark ? "bg-slate-800 border-slate-700" : "bg-slate-50 border-slate-200"}`}>
            <div className="grid grid-cols-[1fr_40px_40px_40px] items-center gap-2">
              <div className={`text-xs font-semibold ${isDark ? "text-white" : "text-slate-700"}`}>Parça</div>
              <div className={`text-[10px] text-center ${isDark ? "text-slate-300" : "text-slate-500"}`}>Ori</div>
              <div className={`text-[10px] text-center ${isDark ? "text-slate-300" : "text-slate-500"}`}>Boy</div>
              <div className={`text-[10px] text-center ${isDark ? "text-slate-300" : "text-slate-500"}`}>Değ</div>
            </div>
          </div>
                ...
                >
                  <div className="grid grid-cols-[1fr_40px_40px_40px] items-center gap-2 px-2 py-1.5">
                    <div className="min-w-0">
                      <div className={`text-[12px] font-medium truncate ${isDark ? "text-white" : "text-slate-900"}`}>
                        {part.label}
                      </div>
          ...
          <div className={`mt-auto px-3 py-2 border-t text-[10px] flex items-center justify-between ${
